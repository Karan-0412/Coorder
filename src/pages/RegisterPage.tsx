...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setIsLoading(true);
    try {
      // username derived from name — backend can adjust
      const username = form.name.toLowerCase().replace(/\s+/g, '_');
      await register({ name: form.name, username, email: form.email, password: form.password });
    ...
    <div className="min-h-dvh bg-[#fef4f1] dark:bg-background text-on-surface flex flex-col items-center transition-colors duration-300">
      ...
      <main className="w-full flex-grow flex items-center justify-center p-gutter lg:p-xl relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden" aria-hidden>
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-primary opacity-[0.04] rounded-full blur-[100px]" />
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-secondary opacity-[0.04] rounded-full blur-[100px]" />
        </div>
          ...
          <div className="bg-surface-container-lowest dark:bg-surface-container shadow-[0_20px_40px_-12px_rgba(173,44,0,0.12)] rounded-xl border border-outline-variant/30 p-xl text-center transition-colors duration-300">
            ...
            <div className="mb-lg">
              ...
              <p className="font-body-md text-body-md text-on-surface-variant mt-1">
                Start your community group-buy journey today.
              </p>
            </div>
            ...
            <form className="space-y-md text-left" onSubmit={handleSubmit} noValidate>
              ...
              <div className="space-y-xs">
                <label className="font-label-bold text-label-bold text-on-surface-variant px-1" htmlFor="name">
                  Full Name
                </label>
              ...
              {/* Password */}
              <div className="space-y-xs">
                <label className="font-label-bold text-label-bold text-on-surface-variant px-1" htmlFor="password">
                  Password
                </label>
                <div className="relative group">
                  <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-primary/50 group-focus-within:text-primary text-[20px] transition-colors">
              ...
              <div className="flex items-start gap-3 py-1">
                <input
                  ...
                  className="mt-0.5 w-4 h-4 accent-primary border-2 border-outline-variant/60 rounded cursor-pointer"
                />
                <label htmlFor="terms" className="font-body-md text-body-md text-on-surface-variant leading-relaxed cursor-pointer">
                  I agree to the{' '}
              ...
              <button
                type="submit"
                disabled={!agreed || isLoading || !form.name || !form.email || !form.password}
            ...
            <div className="mt-lg pt-lg border-t border-outline-variant/30">
              <div className="flex items-center justify-center gap-2 mb-3">
                <div className="flex -space-x-2">
                  <div className="w-6 h-6 rounded-full border-2 border-surface-container-lowest bg-secondary-fixed-dim" />
                  <div className="w-6 h-6 rounded-full border-2 border-surface-container-lowest bg-primary-fixed-dim" />
                  <div className="w-6 h-6 rounded-full border-2 border-surface-container-lowest bg-tertiary-fixed-dim" />
                </div>
                <span className="font-label-bold text-label-bold text-primary">Join 10k+ neighbors</span>
              </div>
